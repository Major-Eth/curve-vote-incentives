...
function Voting() {
	...
	const [rewardsByType, set_rewardsByType] = useState({
		claimable: [],
		potential: []
	});
	const [voteRewardsByType, set_voteRewardsByType] = useState({
		claimable: [],
		potential: []
	});
	...
	useEffect(() => {
		const _claimable = rewards
			.filter((r) =>
				search === ''
					? true
					: (r?.rewardToken?.address || '').toLowerCase() === search.toLowerCase()
			)
		...
		const _potential = rewards
			...
			.filter(
				(reward) =>
					...
					ethers.BigNumber.from(reward.rewardsUnlock).gt(
						moment().unix()
					)
			)
			.sort((a, b) => {
	...
	useEffect(() => {
		const _claimable = voteRewards.filter(
			(reward) =>
				reward.voterState === 1 && reward.vote.vote.open !== true
		);
		const _potential = voteRewards
			.filter((reward) => reward.vote.vote.open === true)
			.sort((a, b) => {
	...
	if (!active) {
		return (
			<div>
				<div
					className={
						'flex flex-col items-center px-28 mt-48 w-full h-full'
					}
				>
					<p className={'text-2xl font-semibold'}>
						{'Connect your wallet to find your rewards.'}
					</p>
					<WalletSelection />
				</div>
			</div>
	...
	return (
		<div>
			<div
				className={
					'flex sticky top-0 z-10 justify-between items-center w-full h-20 bg-white'
				}
			>
				<Header>
					<div
						className={'flex justify-end items-center mr-16 w-full'}
					>
						<input
							className={
							...
							placeholder={
								'Reward Token Address (eg. 0x6b1754....1d0f)'
							}
							value={search}
			...
			{!rewardsByType?.claimable?.length &&
				!voteRewardsByType?.length &&
				!rewardsByType?.potential?.length &&
				!voteRewardsByType?.potential?.length &&
				search === '' && (
				<section className={'px-20 mt-10'}>
					<p className={'text-base text-dark-blue-1'}>
						{'Loading Rewards:'}
					</p>
					<div className={'grid grid-cols-3 gap-8 mt-4'}>
						<div
							className={
								'flex flex-col items-center py-6 px-4 h-80 bg-white rounded-md shadow-sm animate-pulse'
							}
						/>
						<div
							className={
			...
			{(rewardsByType.claimable.length > 0 ||
				voteRewardsByType.length > 0) && (
				<section className={'px-20 mt-10'}>
					<p className={'text-base text-dark-blue-1'}>
						{'Claimable Rewards:'}
					</p>
			...
			{(rewardsByType.potential.length > 0 ||
				voteRewardsByType.potential.length > 0) && (
				<section className={'px-20 mt-10'}>
					<p className={'text-base text-dark-blue-1'}>
						{'Upcoming Rewards:'}
					</p>
					<div
						className={
							'grid grid-cols-1 gap-8 mt-4 md:grid-cols-2 xl:grid-cols-3'
						}
					>
