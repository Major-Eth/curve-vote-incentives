		...
		<div className={'flex flex-col w-full h-full'}>
			...
			>
				...
				<p className={'text-base text-gray-blue-2'}>
					{
						'Choose a proposal that you would like to support below...'
					}
				</p>
			</div>
				...
				<div className={'w-full'}>
					{votes &&
						...
						votes
							...
							.filter((vote) => {
								if (search) return vote.index == search;
								return true;
							})
							.map((vote, index) => {
								const yesPerc =
									Number(vote.vote.yea) /
									(Number(vote.vote.yea) +
										Number(vote.vote.nay));
								const quorumPerc =
									(Number(vote.vote.yea) +
										Number(vote.vote.nay)) /
									Number(vote.vote.votingPower);
								...
								return (
									...
									>
										...
										<div className={'col-span-5'}>
											<div
												className={
													'overflow-hidden relative mb-1 w-11/12 h-5 bg-yearn-blue-light-1 rounded-sm'
												}
											>
												<div
													className={
														'absolute inset-y-0 left-0 h-full bg-yearn-blue'
													}
													style={{
														width: `${yesPerc}%`
													}}
												...
												<div
													className={
														'absolute inset-y-0 w-0.5 h-full bg-dark-blue-3'
													}
													style={{
														left: `${ethers.utils.formatUnits(
															vote.vote
																.supportRequired,
															16
														)}%`
										...
										<div
											className={
												'col-span-2 justify-end text-right'
											}
										>
											<p
												className={
