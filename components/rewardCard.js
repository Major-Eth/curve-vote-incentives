	...
	const renderAvailable = () => {
		return (
			<>
				...
				<p className={'my-3 text-3xl font-bold text-center text-dark-blue-1'} >
					{`${formatCurrency(Number(reward.tokensForBribe) * (reward.gauge.votes.userVoteSlopePercent) / 100)} ${reward.rewardToken.symbol}`}
				</p>
				<p className={'text-xs text-center text-gray-blue-1'}>
					{`100% vote for ${reward.gauge.name} gives you ${formatCurrency(Number(reward.tokensForBribe))} ${reward.rewardToken.symbol}`}
				</p>
